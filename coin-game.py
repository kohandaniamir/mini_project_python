...
while running:
    ...
    if not game_over:
        ...
        if player_x < obstacle_x + obstacle_width and player_x + player_width > obstacle_x and player_y < obstacle_y + \
            obstacle_height and player_y + player_height > obstacle_y:
            game_over = True

        if player_x < coin_x + coin_radius and player_x + player_width > coin_x - coin_radius and player_y < coin_y + \
                coin_radius and player_y + player_height > coin_y - coin_radius:
            score += 10
            coin_x = random.randint(coin_radius, screen_width - coin_radius)
            coin_y = -coin_radius
    if dark_mode:
